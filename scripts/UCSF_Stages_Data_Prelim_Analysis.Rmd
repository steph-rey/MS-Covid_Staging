...
create_report(data = df, 
              output_file = "Demographics_and_Daily_Covid_Stage_Data_Profile_Report",
              output_dir = "/Users/sreynolds2/Documents/GitHub/MS-Covid_Staging/reports/",
              report_title = "EDA Report for Demographics and Daily Covid Stage Data - Sys.time()",
             # y = 
)
...
```{r Correlation matrix with ppssr}
df %>% select(-c(ID, zip)) %>% visualize_pps(y = 'LOS', do_parallel= TRUE)
```
...
# Create EDA report in HTML format 
ExpReport(data = df,
...
```{r EDA with dlookr}
library(dlookr)
diagnose_paged_report(df, output_file = "pdf", create_date = format(Sys.time(), '%b %d, %Y  %H:%M %p'), subtitle = "Demographics and Daily Covid Stage Data", author = "Steph Reynolds (Stephanie.Reynolds@ucsf.edu)", abstract = "This report was created for an overview quality diagnosis of demographics and daily Covid staging data. It was created for the purpose of judging the validity of variables before conducting EDA.", flag_content_zero = FALSE, flag_content_minus = FALSE)
```
